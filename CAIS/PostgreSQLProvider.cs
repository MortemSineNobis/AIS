...
namespace CAIS
{
    ...
    {
        private const int MAXDictionary = 1000;

        Dictionary<string, long> vessels = new Dictionary<string, long>();
        ...
        public PostgreSQLProvider(string? Host = null, int? Port = null, string? Username = null, string? Password = null, string? Database = null, int? ais = null)
        {
            if (!string.IsNullOrEmpty(Host))
                this.Host = Host;
            this.Port = Port ?? 5432;
        ...
        {
            long id = -1;
            if (vessels.TryGetValue(mmsi, out id))
                return id;
            else
            {
                NpgsqlCommand cmd1;
        ...
        private void AddVesselToDictiomary(string mmsi, long id)
        {
            if (vessels.Count >= MAXDictionary)
                vessels.Clear();
            vessels.Add(mmsi, id);
        }
        ...
        {
            var sql = $"INSERT INTO public.nmea(\"type\", \"raw\", vessel, \"data\", \"AIS\") " +
                $"VALUES('{nmea.ais_id}', '{nmea.raw}', '{vessel}', '{dopvalue}', '{AIS}');";
            cmd.CommandText = sql;
            var res = cmd.ExecuteScalar();
